...
class WP_Mapbox_GL_JS_Admin {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style('wp_mapbox_gl_js_mapbox_gl_js_css', 'https://api.mapbox.com/mapbox-gl-js/v1.12.0/mapbox-gl.css');
	 ...
	 * @since    1.0.0
	 */
	function wp_mapbox_gl_js_upgrade_notice() {
			if(!get_option( 'wp_mapbox_gl_js_upgrade' )) {
				update_option( 'wp_mapbox_gl_js_upgrade', true );
			?>
		    <div class="notice notice-success is-dismissible">
		      <img style="width: 50px;margin-top:10px;float:left;margin-right: 10px;" src="<?php echo plugins_url(); ?>/wp-mapbox-gl-js/admin/img/logo-Mapster.png" />
					<h2>We've replaced this plugin with <a href="https://wordpress.org/plugins/mapster-wp-maps" target="_blank">a new one</a>!</h2>
					<p>Check out Mapster WP Maps for all your Mapbox mapping needs. You can even import from this old plugin to our new one!</p>
					<p><a href="https://wpmaps.mapster.me/" target="_blank">Learn More</a></p>
		    </div>
	    <?php }
	}
